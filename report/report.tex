...
Profiling was completed using IPython's magic commands $\texttt{\%timeit}$, $\texttt{\%lprun}$, and $\texttt{\%mprun}$. By timing the $\texttt{\%dynamics}$ function calls with differing $\texttt{\%dt}$ input, the result came out to be just over 300 ms per $\texttt{\%dt}$ per site. This result is sufficient for the purposes of perfroming simulations.
